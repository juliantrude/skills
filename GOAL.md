...
STATUS: READY
**Next action:** Build the **logs** page: the full run log, with the newest lines visible without scrolling (Routing and pages, 4th/last increment). `#page-logs` already renders `api/log` into a `<pre>` verbatim (from the routing increment); the remaining work is "newest lines visible without scrolling" — check current behaviour against a long log and decide whether that needs reversing the order, scrolling-to-bottom, or a max-height with anchor-to-end, then verify by rendering with a long fixture log.
**Blockers:** none
